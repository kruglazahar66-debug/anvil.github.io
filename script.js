...
window.showAdminPanel = function() {
    ...
    let matchesListHtml = matches.map(m => `
        <div class="match-item">
            ...
            <div class="match-actions">
                ...
                <button class="delete-btn" onclick="window.deleteMatchAdmin('${m.id}')"><i class="fas fa-trash"></i> Уд.</button>
            </div>
        </div>
    `).join('');
    ...
    app.innerHTML = `
        <div class="admin-panel">
            <h2><i class="fas fa-crown"></i> Админ панель ANVIL</h2>
            <div class="admin-form">
                <h3 style="margin-bottom: 20px;"><i class="fas fa-plus-circle"></i> Добавить событие</h3>
                ...
                <div class="form-group">
                    <label>URL трансляции (YouTube, Twitch, Vimeo)</label>
                    <input type="text" id="newUrl" placeholder="https://www.youtube.com/watch?v=... или https://www.twitch.tv/...">
                </div>
                <div class="form-group">
                    <label>Краткий прогноз / описание</label>
                    <input type="text" id="newPreview" placeholder="Прогноз: 2:1, фаворит - команда А">
                </div>
                <button class="admin-btn" onclick="window.addMatchFromAdmin()"><i class="fas fa-save"></i> Добавить событие</button>
            </div>
            <h3 style="margin-bottom: 20px;"><i class="fas fa-list"></i> Управление событиями</h3>
            <div id="matchesListAdmin">${matchesListHtml || '<p>Нет событий</p>'}</div>
        </div>
        <button onclick="window.showMatchesView()" class="nav-btn" style="margin-top: 30px;"><i class="fas fa-arrow-left"></i> На главную</button>
    `;
};
...
window.addMatchFromAdmin = function() {
    ...
    if (!title || !datetime || !url) {
        alert('Заполните название, дату и URL!');
        return;
    }
...
window.editMatch = function(id) {
    ...
    const newPreview = prompt('Новый прогноз/описание:', match.preview);
    if (newTitle && newDatetime && newUrl) {
        updateMatch(id, newTitle, newDatetime, newUrl, newPreview);
    }
};
...
window.showAuth = function() {
    ...
    app.innerHTML = `
        <div class="auth-container" id="authForm">
            ...
            <div class="form-group">
                <label>Логин</label>
                <input type="text" id="loginInput" class="auth-input" placeholder="admin / user">
            </div>
            <div class="form-group">
                <label>Пароль</label>
                <input type="password" id="passwordInput" class="auth-input" placeholder="••••••">
            </div>
            <div class="form-group" id="nameGroup" style="display:none;">
                <label>Ваше имя</label>
                <input type="text" id="nameInput" class="auth-input" placeholder="Имя">
            </div>
            <button id="submitAuthBtn" class="auth-btn">Войти</button>
